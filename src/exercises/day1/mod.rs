...
pub fn find_three_that_sum_to(numbers: &[u32], sum_to: u32) -> Option<ThreeNumbersAndTheirProduct> {
    for (index, number) in numbers.iter().filter(|num| num < &&sum_to).enumerate() {
        let difference = &sum_to - number;
        // check to see if `difference` is in `numbers` with this `number` ignored.
        if let Some((number_b, number_c, product)) =
            find_two_that_sum_to(&numbers[index + 1..], difference)
        {
...
#[cfg(test)]
mod tests {
    use crate::exercises::day1::{find_three_that_sum_to, find_two_that_sum_to};
    use std::fs::File;
    use std::io::Read;

    fn read_input() -> Vec<u32> {
        let mut file =
            File::open("src/exercises/day1/input.txt").expect("File exists and can be opened");
    ...
    fn day_1b_works() {
        let numbers = read_input();
        let output =
            find_three_that_sum_to(&numbers, 2020).expect("There to be three such numbers");
